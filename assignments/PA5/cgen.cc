...
void CgenClassTable::code_constants()
{
    ...
    stringtable.code_string_table(str, TAG_STR);
    inttable.code_string_table(str, TAG_INT);
    code_bools(TAG_BOOL);
}
...
void CgenClassTable::code()
{
    ...
    if (cgen_debug)
        cout << "coding class_name table" << endl;
    ...
    if (cgen_debug)
        cout << "coding object constructors" << endl;
    code_object_constructors();
...
void CgenClassTable::code_class_name_table()
{
    ...
    for (auto l : staticNds)
        ...
        {
            str << WORD;
            stringtable.lookup_string(l->get_name()->get_string())->code_ref(str);
            str << endl;
        }
}
void CgenClassTable::code_object_table()
{
    ...
    for (auto l : staticNds)
        if (l)
        {
            str << WORD << l->get_name() << PROTOBJ_SUFFIX << endl
                << WORD << l->get_name() << CLASSINIT_SUFFIX << endl;
        }
}
void CgenNode::code_dispatch_table(std::ostream &str)
{
    for (auto &&method : _methods)
    {
        str << WORD << method.type << METHOD_SEP << method.name << endl;
    }
}
void CgenClassTable::code_dispatch_tables()
{
    for (auto l : staticNds)
        if (l)
        {
            str << l->get_name() << DISPTAB_SUFFIX << LABEL;
            l->code_dispatch_table(str);
        }
}
void CgenNode::code_prototype(std::ostream &str)
{
    if (get_parent() != No_class)
        get_parentnd()->code_prototype(str);
    if (get_name() == Str)
        str << WORD << "Int_protObj" << endl
            << WORD << 0 << endl;
    else
...
void CgenClassTable::code_prototypes()
{
    for (auto l : staticNds)
        ...
        {
            str << WORD << "-1" << endl
                << l->get_name() << PROTOBJ_SUFFIX << LABEL
                << WORD << l->getTag() << endl
                << WORD << l->getSize() << endl
                << WORD << l->get_name() << DISPTAB_SUFFIX << endl;
            l->code_prototype(str);
        }
}
void CgenClassTable::code_prepare()
{
...
void CgenClassTable::code_object_constructors()
{
    for (auto l : staticNds)
        ...
        {
            ndSelf = l;
            str << l->get_name() << CLASSINIT_SUFFIX << LABEL
                << ADDIU << "$sp $sp -8" << endl
                << SW << "$s0 8($sp)" << endl
                << SW << "$ra 4($sp)" << endl
                << MOVE << "$s0 $a0" << endl;

            if (l->get_parent() != No_class)
...
void CgenClassTable::code_object_methods()
{
    for (auto l : staticNds)
        if (l)
        {
            ...
            {
                auto method = dynamic_cast<method_class *>(l->features->nth(i));
                if (!method)
                    continue;
                stack.clear();

                for (int i = 0; i < method->formals->len(); ++i)
                    stack.push(method->formals->nth(i)->getName());
                stack.sentry(2);
                str << l->get_name() << METHOD_SEP << method->name << LABEL
                    << ADDIU << "$sp $sp -8" << endl
                    << SW << "$s0 8($sp)" << endl
                    << SW << "$ra 4($sp)" << endl
                    << MOVE << "$s0 $a0" << endl;
                ...
                str << LW << "$ra 4($sp)" << endl
                    << LW << "$s0 8($sp)" << endl
                    << ADDIU << "$sp $sp " << 8 + 4 * method->formals->len() << endl
                    << RET << endl;
            }
        }
}
...
void assign_class::code(ostream &s)
{
    ...
    int off = stack[name];
    if (off)
        s << SW << "$a0 " << 4 * off << "($sp)" << endl;
    else
        s << SW << "$a0 " << 4 * ndSelf->getFields()[name]->off << "($s0)" << endl;
}
...
void static_dispatch_class::code(ostream &s)
{
    ...
    s << BNE << "$a0 $zero label" << label << endl
      << LA << "$a0 str_const" << filestr << endl // dead
      << LI << "$t1 " << line_number << endl      // dead
      << JAL << "_dispatch_abort" << endl
      << "label" << label << LABEL
      << LA << "$t0 " << type_name << DISPTAB_SUFFIX << endl
      << LW << "$t0 " << 4 * off << "($t0)" << endl
      << JALR << "$t0" << endl;
    ++label;
    stack.pop_sentry();
...
{
    int l1 = label, l2 = label + 1;
    label += 2;
    pred->code(s);
    s << LW << "$t0 12($a0) " << endl
      << BEQ << "$t0 $zero label" << l1 << endl;
    then_exp->code(s);
    s << BRANCH << "label" << l2 << endl
      << "label" << l1 << LABEL;
    else_exp->code(s);
    s << "label" << l2 << LABEL;
}
...
void loop_class::code(ostream &s)
{
    ...
    pred->code(s);
    s << LW << "$t0 12($a0) " << endl
      << BEQ << "$t0 $zero label" << l2 << endl;
    body->code(s);
    s << BRANCH << "label" << l1 << endl
      << "label" << l2 << LABEL;
}
...
void branch_class::code(ostream &s)
{
    ...
    expr->code(s);
    stack.pop();
    s << BRANCH << "label" << caseLabel.back() << endl
      << "label" << l << LABEL;
}
...
void new__class::code(ostream &s) //
{
    ...
    {
        s << LA << "$a0 " << type_name << PROTOBJ_SUFFIX << endl
          << JAL << "Object.copy" << endl
          << JAL << type_name << CLASSINIT_SUFFIX << endl;
    }
}
...
void object_class::code(ostream &s)
{
    ...
    if (name == self)
        ...
        s << LW << "$a0 " << 4 * off << "($sp)" << endl;
    else
        s << LW << "$a0 " << 4 * ndSelf->getFields()[name]->off << "($s0)" << endl;
}
